...
async function execute(interaction) {
	...
	for ( i = 0; i < this.date_thresholds.length; i++ )
	{
		if (epoch >= this.date_thresholds[i][0] && epoch < this.date_thresholds[i][1])
		{
			if (track === "app_dev")
			{
				await interaction.reply({content: nowMsg + ad_links[i][now.getDay()] + "\n" + nextMsg + ad_links[i+1][now.getDay()], ephemeral: true});
			}
			if (track === "qa")
			{
				await interaction.reply({content: nowMsg + qa_links[i][now.getDay()] + "\n" + nextMsg + qa_links[i+1][now.getDay()], ephemeral: true});
			}
			return;
		}
	}
